import { eventsData, renderEvents } from "./events.js";
...
function renderCalendar(month, year, allEvents) {
    renderEvents();
    ...
    const thisYearAndMonthEvents = allEvents.filter((event) => event.date.getFullYear() === year && event.date.getMonth() === month);
    for (let i = 0; i < 6; i++) {
        ...
        for (let j = 0; j < 7; j++) {
            if (i === 0 && j < firstDayOfMonth) {
                ...
                if (thisYearAndMonthEvents.length > 0) {
                    const todaysEvents = thisYearAndMonthEvents.filter((event) => event.date.getDate() === date);
